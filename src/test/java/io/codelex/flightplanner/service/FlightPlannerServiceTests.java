...
public class FlightPlannerServiceTests {
    ...
    public void searchFlight(){
        ...
        Mockito.when(flightPlannerInMemoryRepository.getFlights()).thenReturn(flights);

        SearchFlightRequest searchFlightRequest = new SearchFlightRequest(airport1.getAirport(), airport2.getAirport(), LocalDate.now());
        List<Flight> searchResult = flightPlannerInMemoryService.searchFlights(searchFlightRequest).getItems();
